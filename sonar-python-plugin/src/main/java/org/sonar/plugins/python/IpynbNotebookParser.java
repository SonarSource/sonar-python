...
public class IpynbNotebookParser {
  ...
  private NotebookParsingData parseSourceArray(int startLine, JsonParser jParser) throws IOException {
  ...
  private NotebookParsingData parseSourceMultilineString(int startLine, JsonParser jParser) throws IOException {
    NotebookParsingData cellData = NotebookParsingData.fromLine(startLine);
    String sourceLine = jParser.getValueAsString();
    JsonLocation tokenLocation = jParser.currentTokenLocation();
    var previousLen = 0;
