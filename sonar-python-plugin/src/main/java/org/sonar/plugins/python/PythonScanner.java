...
public class PythonScanner extends Scanner {
  ...
  public PythonScanner(
    ...
    ProjectFileWalker projectFileWalker) {
    super(context);
    this.checks = checks;
    ...
    indexer.buildOnce(context, projectFileWalker);
  }
  ...
  protected void scanFile(InputFile inputFile) {
    ...
    try {
      ...
      visitorContext = new PythonVisitorContext(parse, pythonFile, getWorkingDirectory(context), indexer.getPackageNames().get(inputFile.uri()), indexer.getProjectLevelSymbolTable());
