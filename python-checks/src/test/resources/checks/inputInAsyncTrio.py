...
#   ^^^^^> {{This function is async.}}
        name = input()  # Noncompliant {{Wrap this call to input() with await trio.to_thread.run_sync(input).}}
        #      ^^^^^
        return name
# fmt: on


async def trio_with_proper_input():
    name = await trio.to_thread.run_sync(input)  # Compliant
    return name


async def trio_nested_async_with_input():
    async def inner():
