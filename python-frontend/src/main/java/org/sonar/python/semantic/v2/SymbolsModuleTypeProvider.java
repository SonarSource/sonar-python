...
public class SymbolsModuleTypeProvider {
  ...
  private Optional<ModuleType> createModuleTypeFromProjectLevelSymbolTable(String moduleName, String moduleFqn, ModuleType parent) {
    var retr = projectLevelSymbolTable.getDescriptorsFromModule(moduleFqn);
    if (retr == null) {
      return Optional.empty();
    }
    var members = retr.stream().collect(Collectors.toMap(Descriptor::name, anyDescriptorToPythonTypeConverter::convert));
